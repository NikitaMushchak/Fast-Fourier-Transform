...
void calculatePressure(
    ...
    pressure.resize(InfluenceVector.size());
    // std::cout<<"....";
    std::fill(pressure.begin(), pressure.end(), 0);
// std::cout<<"calc pressure";
    std::vector<std::vector<double>> pressureS;
    pressureS.resize(InfluenceVector.size());
    for(size_t i = 0; i < InfluenceVector.size(); ++i){
        pressureS[i].resize(2);
        pressureS[i][0] = 0.;
        pressureS[i][1] = 0.;
    }
    // std::cout<<"pressureS size =" << pressureS.size()<<std::endl;
    std::vector<std::vector<double>> product;
        product.resize(pressureS.size());
    ...
    fft2(pressureS);
    getWiseElement2(InfluenceVector, pressureS, product);
    ...
    for(size_t i = 0; i < activeElements.size(); ++i){
        const size_t ai = activeElements[i][0];
        const size_t aj = activeElements[i][1];
       pressure[index[ai][aj]] = product[index[ai][aj]][0] / dx + sigma[i];
      // pressure[index[ai][aj]] = a[index[ai][aj]] / dx + sigma[i];
    }
}

void calculatePressureDir(
    std::vector<double> &pressure,
    std::vector< std::vector<size_t> > &index,
    std::vector< std::vector<size_t> > &activeElements,
    std::vector< std::vector<double> > &influenceMatrix,
...
void calculateOpeningSpeed(
	...
	for (size_t k = 0; k < activeElements.size(); ++k) {
		...
		if (i00 == i) {
			flowIMinusHalf = 0.;//-flowIPlusHalf;
		}
		...
		const double value = xDerivative + yDerivative
			+ 0.25*Q0 * (double)(i == i00  && j == j00) / std::pow(dx, 2)//j00+1
            + 0.25*Q0 * (double)(i == i00  && j == j00 + 1) / std::pow(dx, 2)//j00+1
