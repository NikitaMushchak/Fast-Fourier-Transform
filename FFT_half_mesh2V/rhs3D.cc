...
// std::cout<<"calc pressure";
    ...
    pressureS.resize(InfluenceVector1.size()/2);
    ...
    const size_t aj = activeElements[i][1];
        // pressureS[index[ai][aj]][0] = opening[i];
        pressureS[index[ai][aj]] = opening[i];
        }

    ai::saveVector("./prs",pressureS);

    for (size_t i = pressureS1.size()/2 ; i < pressureS1.size(); ++i)
    {
      pressureS1[i] = pressureS[i - pressureS1.size()/2 ];
    }

    for (size_t i = 0 ; i < pressureS1.size()/2; ++i)
    {
      pressureS1[i] = pressureS[ pressureS1.size()/2 - i ];
    }
    //ai::saveVector("./prsB",pressureS1);
